        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {GATES.map((gate) => (
              ...
              >
                <div className={`${gate.color} w-full aspect-square rounded-lg flex items-center justify-center text-white font-bold text-xl cursor-grab active:cursor-grabbing hover:scale-105 transition-transform neon-violet`}>
                  {gate.name}
                </div>
                <div className="absolute top-full left-0 mt-2 w-40 glass p-2 rounded-md opacity-0 group-hover:opacity-100 transition-opacity z-50 pointer-events-none text-[10px]">
      ...
      <Card className="lg:col-span-3 glass border-white/5 overflow-hidden flex flex-col relative">
        <div className="p-4 border-b border-white/5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Badge variant="secondary" className="bg-primary/10 text-primary">Live Simulator</Badge>
            <span className="text-xs text-muted-foreground">Drag gates onto the qubit lines</span>
          </div>
        ...
        <div className="flex-1 overflow-auto p-12 bg-black/20">
          <div className="flex flex-col gap-16 min-w-[800px]">
            {qubits.map((q) => (
              <div key={q} className="relative flex items-center group h-12">
                <span className="absolute -left-8 font-mono text-sm text-accent tracking-tighter w-8">q[{q}]</span>
                ...
                <div className="flex gap-4 ml-4">
                  {[0, 1, 2, 3, 4, 5, 6, 7].map((step) => {
                    ...
                    return (
                      <div 
                        key={step} 
                        className={`w-12 h-12 rounded-md border-2 border-dashed border-white/5 hover:border-primary/50 transition-all flex items-center justify-center relative z-10 cursor-pointer ${gate ? 'border-none' : ''}`}
                      ...
                      >
                        {gate && (
                          <div className={`${gate.color} w-full h-full rounded flex items-center justify-center text-white font-bold shadow-lg animate-in fade-in zoom-in duration-300`}>
                            {gate.name}
                          </div>
                        )}
                      </div>
